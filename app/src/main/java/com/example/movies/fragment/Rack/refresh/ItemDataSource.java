...
public class ItemDataSource extends PageKeyedDataSource<Integer, Movie> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Movie> callback) {
        Retroficlient.getRetrofit().create(api.class).getRankMovies(FIRST_PAGE).enqueue(
                new Callback<MovieModel>() {
            @Override
            public void onResponse(Call<MovieModel> call, Response<MovieModel> response) {
                Ranck.swipeRefreshLayout.setRefreshing(false);
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Movie> callback) {
        Ranck.swipeRefreshLayout.setRefreshing(true);

        Retroficlient.getRetrofit().create(api.class).getRankMovies(params.key)
